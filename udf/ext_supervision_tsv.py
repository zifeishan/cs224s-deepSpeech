...
for row in sys.stdin:
  ...
  lattice_id, starts, ends, candidates, candidate_ids, transcript = parts
  ...
  # Heuristics: <s> to start and </s> to end?
  ...
  for startid in start_ids:
    ...
    for path in DFS(lattice_id, edges, startid, res_array, index_cid_sub, end_ids):
      ...
      # # DEBUG
      # print path  
      # print >>sys.stderr, 'Path %5d: %s' % (pathnum, ' '.join(path_words))
      # print >>sys.stderr,'Differences: +',set(path).difference(debug_lastpath), '-', debug_lastpath.difference(path)
      # debug_lastpath = set(path)
      # print >>sys.stderr, 'Path %5d: %s' % (pathnum, ' '.join([str(_) for _ in path]))
      # time.sleep(0.5)
  ...
  print >>sys.stderr, '%d paths, true labels: %d / %d' % (pathnum, len(true_cids), len(true_cids) + len(false_cids) )
